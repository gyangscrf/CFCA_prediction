function [regfd, Wfd, shift, Fstr, iternum, interhist] = ...
...
nbasis0 = getnbasis(basis0);
type0   = getbasistype(basis0);
...
if periodic
   if ~getbasistype(ybasis) == 'fourier'
...
ncoef  = wdim(1);
ndimw  = length(wdim);
if wdim(ndimw) == 1, ndimw = ndimw - 1; end;
if ndimw == 1
   if ncurve > 1
...
for icurve = 1:ncurve
  ...
  yfine = squeeze(eval_fd(yfdi, xfine));
