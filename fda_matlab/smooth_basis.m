...
if n >= nbasis | lambda > 0
    ...
    if lambda > 0
        %  it was thought better of using forcing functions in
        %  the definition of a linear differential operator object.
...
%                 ufdk = ufdcell{k};
%                 ubasis = getbasis(ufdk);
%                 if strcmp(getbasistype(ubasis), 'bspline') & ...
%                         getnbasis(ubasis) - length(getbasispar(ubasis)) == 1
%                     range   = getbasisrange(ubasis); 
%                     breaks  = [range(1), getbasispar(ubasis), range(2)];
%                     nbreaks = length(breaks);
%                     for ibreak=2:nbreaks
%                         uval = ...
%                             eval_fd((breaks(ibreak)+breaks(ibreak-1))/2, ufdk);
%                         if uval ~= 0
%                             rangei = [breaks(ibreak-1),breaks(ibreak)];
%                             auval = inprod(basisobj, uval.*afdk, Lfdhom, int2Lfd(0), rangei);  
%                             penvec = penvec - auval;  
%                         end
%                     end
%                 else
